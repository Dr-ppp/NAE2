...
import co.neeve.nae2.common.interfaces.IExtremeStorageCell;
...
public class ExtremeCellInventory<T extends IAEStack<T>> extends AbstractCellInventory<T> {
    private final IStorageChannel<T> channel;

    private final IExtremeStorageCell<T> type;

    private ExtremeCellInventory(IExtremeStorageCell<T> type, ItemStack o, ISaveProvider container) {
    ...
    public static ICellInventory<?> createInventory(ItemStack o, ISaveProvider container) {
        try {
            if (o == null) {
                ...
                Item type = o.getItem();
                if (type instanceof IExtremeStorageCell<?> cellType) {
                    if (!cellType.isStorageCell(o)) {
                        throw new AppEngException("ItemStack was used as a cell, but was not a cell!");
                    } else {
                        return new ExtremeCellInventory<>(cellType, o, container);
                    }
                } else {
                    throw new AppEngException("ItemStack was used as a cell, but was not a cell!");
                }
            ...
            NAE2.logger().error(exception);
            return null;
        }
    }

    private boolean isExtremeStorageCell(T input) {
        if (!(input instanceof IAEItemStack stack)) {
            return false;
        } else {
            IExtremeStorageCell<?> type = getStorageCell(stack.getDefinition());
    ...
    private static IExtremeStorageCell<?> getStorageCell(ItemStack input) {
        if (input != null) {
            Item type = input.getItem();
            if (type instanceof IExtremeStorageCell) {
                return (IExtremeStorageCell<?>) type;
            }
        }
    ...
    protected boolean loadCellItem(NBTTagCompound compoundTag, long stackSize) {
        ...
        try {
            ...
            if (t == null) {
                NAE2.logger().warn("Removing item " + compoundTag + " from storage cell because the associated item type couldn't be found.", new Object[0]);
                return false;
            }
        } catch (Throwable exception) {
            if (AEConfig.instance().isRemoveCrashingItemsOnLoad()) {
                NAE2.logger().warn("Removing item " + compoundTag + " from storage cell because loading the ItemStack crashed.", exception);
                return false;
            }
    ...
    private static class Adapter<T extends IAEStack<T>> implements IStorageCell<T> {
        private final IExtremeStorageCell<T> cellType;

        public Adapter(IExtremeStorageCell<T> type) {
            cellType = type;
        }

        @Override
        public int getBytes(@NotNull ItemStack itemStack) {
            NAE2.logger().warn("A method that should not have been called was called.");
        ...
        @Override
        public boolean isBlackListed(@NotNull ItemStack itemStack, @NotNull T t) {
        ...
        @Override
        public boolean isStorageCell(@NotNull ItemStack itemStack) {
